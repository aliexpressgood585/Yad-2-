...
const CAR_CONDITION_TEXT = [
  "הרכב במצב מכני ואסתטי מצוין, טופל תמיד במוסך מורשה ויש תיעוד מלא של כל הטיפולים.",
  "שמור מאוד, לא עבר תאונות, צמיגים חדשים והכול עובד כמו שצריך.",
...
function generateCar(rng: Rng, sub: string): GeneratedListing {
  ...
  const hand = Math.min(5, Math.max(0, Math.floor(age / 3) + rng.int(0, 1)));
  ...
  const attrs = clean({
    ...
    testUntil: `${String(rng.int(1, 12)).padStart(2, "0")}/${CURRENT_YEAR + rng.int(0, 1)}`,
    extras: rng.picks(
...
export const HEBREW_FIRST_NAMES = [
  "יעל", "נועם", "איתי", "שירה", "אבי", "מיכל", "דניאל", "רותם", "עומר", "טל",
  "ליאור", "הדר", "אורי", "מאיה", "גיא", "נטע", "יונתן", "שני", "אלון", "עדי",
  "רון", "ספיר", "אסף", "ליה", "תומר", "אורית", "אמיר", "דנה", "נדב", "מור",
  "יוסי", "רבקה", "מוחמד", "פאטמה", "סרגיי", "אנה",
];

export const HEBREW_LAST_NAMES = [
  "כהן", "לוי", "מזרחי", "פרץ", "ביטון", "דהן", "אברהם", "פרידמן", "שפירא", "אזולאי",
  "חדד", "גבאי", "מלכה", "אוחיון", "ברוך", "רוזנברג", "שמש", "נחום", "טל", "בן דוד",
];
