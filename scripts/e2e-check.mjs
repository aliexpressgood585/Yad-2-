 ...
 *
 * שני משתני סביבה נדרשים, וראו את הבדיקה מיד אחרי ההערה הזאת:
 *   E2E_CATEGORY_ID — מזהה תת-קטגוריה עם שדות דינמיים
 *   E2E_SERVER_LOG  — הנתיב ללוג השרת, שממנו נקרא קוד האימות
 */
const BASE = process.argv[2] ?? "http://localhost:3112";

/*
 * בלי המשתנים האלה הבדיקה אינה נכשלת — היא **משקרת**.
 *
 * `E2E_CATEGORY_ID` חסר הופך את הקריאה ל-`/api/categories/undefined`,
 * שמחזירה אפס שדות; משם הפרסום נכשל, ואחריו נכשלות כל הבדיקות
 * שתלויות במודעה שנוצרה — חמישה-עשר כשלים שנראים כמו רגרסיה במוצר
 * ואינם אלא משתנה שלא הועבר.
 *
 * זה קרה בפועל ועלה זמן אמיתי. עדיף להיעצר בשורה אחת.
 */
for (const [name, why] of [
  ["E2E_CATEGORY_ID", "מזהה תת-קטגוריה עם שדות דינמיים (למשל private-cars)"],
  ["E2E_SERVER_LOG", "הנתיב ללוג השרת — משם נקרא קוד האימות"],
]) {
  if (!process.env[name]) {
    console.error(`\n\u001b[31m✗\u001b[0m ${name} אינו מוגדר — ${why}.`);
    console.error("  בלעדיו הבדיקה מדווחת על כשלים שאינם קיימים. ראו את ההערה בראש הקובץ.\n");
    process.exit(1);
  }
}
