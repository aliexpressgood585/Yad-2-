...
/**
 * השלב הרחב ביותר שמותר להציג ממנו מספר.
 *
 * שלב 3 מרפה עד "אותו יצרן, שנה ±3" או "אותו אזור, חדרים ±0.5" —
 * זו כבר השוואה רופפת, ומספר שיוצא ממנה נראה בדיוק כמו מספר שיוצא
 * מהשלב ההדוק. במדידה 78% מהמודעות נפלו לשם, כלומר הקריאה שהוצגה
 * הייתה ברובה רופפת מבלי שהמשתמש יכול היה לדעת.
 *
 * **עדיף פחות מספרים אמינים מהרבה מספרים רופפים.** שלב 3 נשאר מוגדר
 * ומחושב — הוא הבסיס לזריעה ולניפוי — אבל לא מוצג. החזרתו היא שינוי
 * של הקבוע הזה בלבד.
 */
export const MAX_DISPLAY_TIER = 2;
