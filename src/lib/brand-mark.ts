/**
 * הסימן של מציאה — גאומטריה אחת, שני מרנדררים.
 *
...
/**
 * קו הבסיס שכל השנתות יושבות עליו.
 *
 * הקווים מצוירים ב-`stroke-linecap: butt` ולא `square`: קצה מרובע
 * מאריך כל קו בחצי ממשקלו לכל כיוון, והמחוג עבה מהשנתות — כלומר הוא
...
/**
 * שנתות ראשיות — כל רבע של הסקאלה, בדיוק כמו
 * `repeating-linear-gradient(... 25%)` ב-`.price-scale`.
 */
export const MARK_MAJOR_TICKS = [2, 8, 14, 20, 26] as const;

/**
 * שנתות משנה — באמצע כל רבע.
 *
 * חסרה כאן אחת: השנתה שבין 20 ל-26. שם עומד המחוג, והוא תופס את
 ...
 *
 * לא על שנתה ראשית — מחוג של מכשיר אמיתי כמעט אף פעם לא נוחת בדיוק על
...
/** גובה השנתות. יחס המשנה לראשית זהה ליחס שב-`.price-scale`. */
const MAJOR_TOP = 7;
const MINOR_TOP = 12;
 ...
 *
 * `compact` — רק השנתות הראשיות, בלי שנתות המשנה.
 *
 * הסימן במלואו הוא תשעה אלמנטים ברוחב 28 יחידות, כלומר רווח של 3
 * יחידות בין שנתה לשנתה. בלוגו של הכותרת (24 פיקסלים) ובפאביקון זה
 * פחות משלושה פיקסלים, והאנטי-אליאסינג ממרח את השורה לכתם אפור.
 *
 * ההסרה היא של שנתות המשנה בלבד — הרווח האפור עולה ל-6 יחידות
 * והשורה נשארת חדה. **המחוג לא זז.** הוא נשאר ב-23, גם כשאין שנתה
 * שתופסת את מקומו, כי המיקום שלו הוא כל המשמעות ולא הצפיפות סביבו.
 *
 * המחוג עצמו לא נפגע מהצפיפות: הוא נבדל מהשנתות בצבע ולא ברווח,
...
export function markSvg({
  ...
  /*
   * שנתות הן שערות ולא עמודות. במשקל שבו הקו רחב כמו חצי מהרווח שלידו
