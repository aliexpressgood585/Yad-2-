...
export const SITE = {
  /*
   * שנתות — הקווים הקטנים על סקאלה של מכשיר מדידה.
   *
   * השם נבחר מפני שהוא **הרכיב המרכזי של המוצר**, פשוטו כמשמעו: לכל
   * מודעה יש סקאלה עם שנתות ומחוג שמראה איפה המחיר שלה יושב מול השוק.
   * הלוגו והפיצ'ר הם אותו אובייקט, וזה מה שהופך אותו לניתן להגנה —
   * מתחרה שיאמץ את השם יצטרך לאמץ גם את המנגנון.
   *
   * "לוח" נזנח מפני שהוא תיאורי ולא שם: אי אפשר לרשום עליו סימן
   * מסחרי, והוא מתחרה במילה הגנרית ביותר בתחום בכל חיפוש בגוגל.
   */
  name: "שנתות",
